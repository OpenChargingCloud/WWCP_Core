                               ...
                               IRemoteEVSE
    {
...

        {
            ...
            this.MaxVoltagePrognoses            = MaxVoltagePrognoses is null
                                                          ...
                                                          : new ReactiveSet<Timestamped<Volt>>(MaxVoltagePrognoses);
...

        {
            ...
            try
            {
                    ...
                    AdminStatus.Value == EVSEAdminStatusType.InternalUse)
                {
                    ...
                    lock (chargingReservations)
                    {
                        ...
                        if (!chargingReservations.TryGetValue(ReservationId, out canceledReservation))
                            return Task.FromResult(
                                       CancelReservationResult.UnknownReservationId(
                                           ReservationId,
                                           Reason
                                       )
                                   );
                    ...
                    result = CancelReservationResult.Success(
                                 ReservationId,
                                 Reason,
                    ...
                    else if (Status.Value == EVSEStatusType.Charging)
                    {
                        ...
                        if (chargingSession?.Id == SessionId)
                        {
                            ...
                            var consumption          = MaxPower.HasValue
                                                           ? WattHour.FromWh(Math.Round(((Decimal) duration.TotalHours) * MaxPower.Value.Value, 2))
                                                           : WattHour.Zero;
